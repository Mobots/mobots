...
void (*protocolHandlers[NUM_IDS])(enum PROTOCOL_IDS id, unsigned char *data, unsigned short size);
...
//     print("Rec header\n");
    while(1)
    {
	int ret = USART1_GetData(buffer + receivedHeaderBytes, sizeof(struct ProtocolHeader) - receivedHeaderBytes);
	if(ret < 0)
	{
...
void protocol_receiveData()
{
	...
	{
	    int ret = USART1_GetData(buffer + received, header->payloadSize - received);
	    if(ret < 0)
	    {
	...
	//check payload crc
	if(header->payloadCRC != protocol_calculateCRC(buffer, header->payloadSize))
	{
...
short unsigned int protocol_calculateCRC(unsigned char* data, short unsigned int size)
{
    return crcSlow(data, size);
}


void protocol_sendData(enum PROTOCOL_IDS id, unsigned char* data, short unsigned int size)
{
    struct ProtocolHeader header;
    ...
    {
	int ret = USART1_SendData(((unsigned char *) &header) + sent, sizeof(struct ProtocolHeader) - sent);
	if(ret < 0)
	{
